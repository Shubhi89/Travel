...
import { getAllUsersForNewsEmail } from "../actions/users.action";
...
export const sendSignUpEmail = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run('send-welcome-email', async () => {
        ...
        const introText = (part && 'text' in part ? part.text : null ) || 'Thanks for joining TradeVerse. You now have the tools to track markets and make smarter moves.'
...
export const sendDailyNewsSummary = inngest.createFunction(
  ...
  async ({step}) => {
    ...
    for(const {user , articles} of results) {
      try {
        const propmt = NEWS_SUMMARY_EMAIL_PROMPT.replace('{{newsData}}' , JSON.stringify(articles, null , 2));
